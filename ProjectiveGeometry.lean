...
-- Possible inefficiences arise because we may check membership in lists multiple times.
...
theorem IsDistinct_equiv (Points : List α) (Lines : List (List α)) :
...
theorem IsAffinePlane_equiv (pl : PointsAndLines α) : IsAffinePlane pl ↔ check_affine_plane pl := by
